...
<Box sx={{display: 'flex', flexDirection: 'row', justifyContent: "space-between", alignItems: "end", mb: 4}}>
<Typography variant="h2" align="left" fontStyle="italic" sx={{
    color: '#ffcc00',
    fontWeight: 600,
    fontSize: '2rem',
    paddingBottom: 0
}}>Understanding the Foundations of Text Encoding</Typography>
    <Typography variant="span" color="textSecondary" gutterBottom>
...
Because of its **ambition to be the final text encoding standard**, Unicode is a **layered** and **multi-faceted system**. Working with Unicode can seem complex until you grasp the different layers, facets and the details of it; this article aims to explore this carefully by throughful analyses, and by the end of it, my goal for you is that you have a clear mental model of Unicode — what it is, why we have and how it works.

The main initial takeaway is this: Unicode **standardizes** the *representation* of text across different languages and platforms. Encodings (like <Term name="UTF8" prefix="UNICODE"/>) implement Unicode by translating its code points into bytes. Each character we see is not just a letter or symbol but a defined code point, encoded into bytes, and then rendered as a glyph.
